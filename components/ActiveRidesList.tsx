...
export function ActiveRidesList() {
  const { user } = useUser();
  ...
  const fetchActiveRides = async () => {
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.detail || 'Failed to fetch active rides');
      }
  ...
  return (
    <View style={styles.container}>
      <Text variant="titleLarge" style={styles.title}>Active Rides</Text>
      <FlatList
        data={rides}
