...
export function ConfirmedRidesList() {
  const { user } = useUser();
  ...
  const fetchConfirmedRides = async () => {
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.detail || 'Failed to fetch confirmed rides');
      }
      ...
      console.error('Error fetching confirmed rides:', err);
  ...
  if (rides.length === 0) {
    return <Text style={styles.emptyText}>No confirmed rides found</Text>;
  }

  return (
    <View style={styles.container}>
      <Text variant="titleLarge" style={styles.title}>Confirmed Rides</Text>
      <FlatList
        data={rides}
