...
export function AwaitingRidesList() {
  const { user } = useUser();
  ...
  const fetchAwaitingRides = async () => {
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.detail || 'Failed to fetch awaiting rides');
      }
      ...
      console.error('Error fetching awaiting rides:', err);
  ...
  if (rides.length === 0) {
    return <Text style={styles.emptyText}>No awaiting rides found</Text>;
  }

  return (
    <View style={styles.container}>
      <Text variant="titleLarge" style={styles.title}>Your Awaiting Rides</Text>
      <FlatList
        data={rides}
